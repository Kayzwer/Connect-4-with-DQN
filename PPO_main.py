...
if __name__ == "__main__":
    env = Connect4()
    agent1 = Agent(.000025, .0001, 16, .99, .97, .2, .05, 10)
    agent2 = Agent(.000025, .0001, 16, .99, .97, .2, .05, 10)
    episodes = 5000000
    ...
    for episode in range(episodes):
        ...
        while not done:
            next_state_agent1, reward1, reward2, done = env.step(action_agent2,
                                                                 -1.)
            agent1.memory.store(state_agent1, state_value_agent1, action_agent1,
                                action_prob_agent1, reward1, done)
            if done:
                agent2.memory.store(state_agent2, state_value_agent2,
                                    action_agent2, action_prob_agent2, reward2,
                                    done)
                break
            ...
            next_state_agent2, reward1, reward2, done = env.step(
                action_agent1, 1.)
            agent2.memory.store(state_agent2, state_value_agent2, action_agent2,
                                action_prob_agent2, reward2, done)
            if done:
                agent1.memory.store(state_agent1, state_value_agent1,
                                    action_agent1, action_prob_agent1, reward1,
                                    done)
                break
